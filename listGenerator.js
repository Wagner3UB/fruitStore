...
export const listCartGenerator = () => {
  ...
  const listCartCreator = (item, index) => {
    ...
    + item.product +'</li><li class="middleProductCart"><button class="addItemToCart" onclick="addItemToCart(' + index + ')">+</button><p>'
    + item.itemToCart + '</p><button onclick="removeQuantity(' 
    + index + ')">-</button></li><li class="middleProductCart cartProductDiscount">'
    + item.discount +'<p>%</p></li><li class="lastProductCart"><p>' 
    + item.getTotal().toFixed(2) + '</p><p>€</p></li></ul>'
  }
