...
//Home display creator
  const listCreator = (item, index) => {
    ...
    + item.quantity +'</p><button onclick="removeQuantity(' + index + ')">-</button></li><li class="col-1 color"><h4>Totale:</h4><p id="totalProduct">'
    + (item.price * item.quantity).toFixed(2) +'</p><p>€</p></li><li class="col-1 add_to_cart"><button id="add_to_cart" onclick="addToCart('+ index +')"><span class="material-icons-outlined">add_shopping_cart</span></button></li></ul>'
  }
...
export const listCartGenerator = () => {
  ...
  const listCartCreator = (item, index) => {
    ...
    + (item.price * item.quantity).toFixed(2) + '</p><p>€</p></li></ul>'
  }
